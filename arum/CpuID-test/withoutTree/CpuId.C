...
CpuId::CpuId()
{
    ...
    /*
     * If test passsed, set the enabled field of our class instance
     */
     ...
     * ECX (in that order).
     */
    if (this->cpuid_enabled) {
				// retrieve CPUID function 0
	...
	printf("vendor = %s\n", this->vendor);
        ...
        if (1 <= this->lsfn) {  /*cpuid not supported if < 1*/
           /*
	    ...
	    printf("edx = 0x%8.8lx\n", edx);
             ...
             * is consistent across processor implementations. An Intel 
